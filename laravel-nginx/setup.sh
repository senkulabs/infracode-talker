...
EXAMPLES:
    ...
    $0 --db-name=myapp --db-user=myuser --db-pass=mypass --with-backup \\
...
cleanup_old_r2_backups() {
    local cutoff_date=$(date -d "$R2_RETENTION_DAYS days ago" '+%Y-%m-%d')

    aws s3api list-objects-v2 --bucket "$R2_BUCKET" --prefix "$R2_BACKUP_PATH/$HOSTNAME/" --endpoint-url="$R2_ENDPOINT" --query "Contents[?LastModified<='$cutoff_date'].Key" --output text | while read -r key; do
